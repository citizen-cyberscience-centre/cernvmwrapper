...
########################################################################
# Modified by G.Wolfe Woodbury <redwolfe@gmail.com>  --  04 Jul 2012
# modified to support virtualbox group setup on Fedora 17
# add to cernvmwrapper fork
...
# 
# Fedora has runuser, which greatly eases the use of VirtualBox for CernVM
...
case "$1" in
  start)
	   ...
	   # Fedora and most Linuxen should run this script as root
           $NOCORE
	   ...
	   # the group vboxusers is hardcoded for now.
	   # TODO: manipulate group setting for all the supported CernVM hosts(?)
           ${RUNUSER} - $BOINCUSER -g vboxusers -c "$NOCORE ; $BOINCEXE $BOINCOPTS >>$LOGFILE 2>>$ERRORLOG" 2>/dev/null > /dev/null &
        fi
        sleep 3  
        PID=`local_pidof $BOINCEXE_NAME`
        if [ $PID ]; then
